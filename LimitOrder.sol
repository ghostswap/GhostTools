...
interface IGHOSTLimitOrdersV1 {
    enum OrderStatus { PENDING, FILLED, CANCELLED }
    enum OrderType { FTM_TOKEN, TOKEN_TOKEN, TOKEN_FTM }
    
    function getRouterAddress() external view returns (address);
    
    function placeFTMTokenOrder(address tokenOut, uint256 targetAmountOut, uint256 minAmountOut) external payable;
    ...
    function placeTokenFTMOrder(address tokenIn, uint256 amountIn, uint256 targetAmountOut, uint256 minAmountOut) external;
...
contract GhostOrderRouter {
    ...
    struct Order {
        ...
        uint256 amountIn;           // FTM Amount in 
    ...
    address public constant WFTM = CHANGE_TO_WFTM_ADDRESS_MAINNET; //change
    ...
    function makeFTMTokenSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut) external payable onlyAuthorized {
        // Swap FTM for wFTM then transfer to pair
        IWFTM(WFTM).deposit{value: amountIn}();
        assert(IWFTM(WFTM).transfer(pair, amountIn));
    ...
    function makeTokenFTMSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut) external onlyAuthorized {
        TransferHelper.safeTransferFrom(
            tokenIn, owner, pair, amountIn
        );
        
        uint balanceBefore = IBEP20(WFTM).balanceOf(address(this));
        _swap(pair, tokenIn, tokenOut, address(this));
        
        uint amountOut = IBEP20(WFTM).balanceOf(address(this)).sub(balanceBefore);
        
        require(amountOut >= minAmountOut, 'GhostRouter: INSUFFICIENT_OUTPUT_AMOUNT');
 ...
 * 
 */
contract GhostLimitOrdersV1 is GhostAuth, IGhostLimitOrdersV1 {
    using SafeMath for uint256;
    ...
    address public constant WFTM = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address constant WFTM_BUSD = 0x1B96B92314C44b159149f7E0303511fB2Fc4774f;
    address constant Ghost = address(0xD7B729ef857Aa773f47D37088A1181bB3fbF0099);
    ...
    GhostOrderRouter GhostRouter;
    
    constructor () {
        factory = router.factory();
        GhostRouter = new GhostOrderRouter();
    }
    ...
    function getRouterAddress() external view override returns (address) {
        return address(GhostRouter);
    }
    
    function placeFTMTokenOrder(address tokenOut, uint256 targetAmountOut, uint256 minAmountOut) external payable override {
        createOrder(msg.sender, OrderType.FTM_TOKEN, WFTM, tokenOut, msg.value, targetAmountOut, minAmountOut, takeFee());
    }
    
    function placeFTMTokenOrderFor(address owner, address tokenOut, uint256 targetAmountOut, uint256 minAmountOut) external payable {
        createOrder(owner, OrderType.FTM_TOKEN, WFTM, tokenOut, msg.value, targetAmountOut, minAmountOut, takeFee());
    }
    ...
    function placeTokenTokenOrder(address tokenIn, uint256 amountIn, address tokenOut, uint256 targetAmountOut, uint256 minAmountOut) external override {
        require(IBEP20(tokenIn).allowance(msg.sender, address(GhostRouter)) >= amountIn, "Not enough allowance for order");
        
        createOrder(msg.sender, OrderType.FTM_TOKEN, WFTM, tokenOut, amountIn, targetAmountOut, minAmountOut, takeFee());
    }
    
    function placeTokenFTMOrder(address tokenIn, uint256 amountIn, uint256 targetAmountOut, uint256 minAmountOut) external override {
        require(IBEP20(tokenIn).allowance(msg.sender, address(GhostRouter)) >= amountIn, "Not enough allowance for order");
        
        createOrder(msg.sender, OrderType.TOKEN_FTM, tokenIn, WFTM, amountIn, targetAmountOut, minAmountOut, takeFee());
    }
    ...
    function _cancelOrder(uint256 orderID) internal {
        ...
        // refund and close
        if(ord.swapType == OrderType.FTM_TOKEN){
    ...
    function unableToFulfil(uint256 orderID) internal view returns (bool) {
        Order memory ord = orders[orderID];
        return IBEP20(ord.tokenIn).balanceOf(ord.owner) < ord.amountIn || IBEP20(ord.tokenIn).allowance(ord.owner, address(GhostRouter)) < ord.amountIn;
    }
    ...
    function makeSwap(Order memory ord) internal returns (bool filled) {
        if(ord.swapType == OrderType.FTM_TOKEN){
            try GhostRouter.makeFTMTokenSwap{value: ord.amountIn}(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }else if(ord.swapType == OrderType.TOKEN_TOKEN){
            try GhostRouter.makeTokenTokenSwap(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }else{ // OrderType.TOKEN_FTM
            try GhostRouter.makeTokenFTMSwap(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }
    }
    ...
    function distributeFees() external {
        uint256 toDistribute = IBEP20(Ghost).balanceOf(address(this)).mul(feeSplit).div(100);
        totalDistributed = totalDistributed.add(toDistribute);
        
        // Distribute to stakers & send to authorized fee taker
        IGhost(Ghost).distribute(toDistribute);
        IBEP20(Ghost).transfer(_owner, IBEP20(Ghost).balanceOf(address(this)));
    }
    
    function getFeeInGHOST() public view returns (uint256) {
        uint256 ghost_FTM = getTokenTokenPrice(WFTM, GHOST);
        uint256 FTM_usd = getTokenTokenPrice(BUSD, WFTM);
        
        uint256 FTMAmt = FTM_usd.mul(fee);
        uint256 ghostAmt = FTMAmt.mul(ghost_FTM).div(10 ** PRICE_DECIMALS).mul(10 ** 18).div(10 ** PRICE_DECIMALS).div(100);
    ...
    function takeFee() internal returns (uint256 amount) {
        ...
        if(msg.sender == tx.origin){
            // Take fee
            amount = getFeeInGHOST();
        }else{
            // Take 0.25 GHOST fee as fiat price could be manipulated
            amount = 250 * (10 ** 16);
        }
        
        IBEP20(GHOST).transferFrom(msg.sender, address(this), amount);
     ...
     * USD per FTM
     */
    function getFTMSpotPrice() external view returns (uint256) {
        // res0 = wFTM, res1 = busd
        (uint112 reserve0, uint112 reserve1,) = IPancakePair(WFTM_BUSD).getReserves();
